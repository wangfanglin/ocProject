	...
	<div class="panel-container">
		...
		<div class="panel-toolbar fix">
			...
			<span class="panel-message-type">
				<span class="article-unread-message color-primary ml10">未读消息</span>
				<span class="article-read-message color-text-secondary">已读消息</span>
			</span>
			<div class="r">
				<el-button type="text" size="small" @click="toNotification">更多</el-button>
				<!-- 操作按钮 -->
		...
		<!-- 主体区域 -->
		<div class="panel-article-main mt10">
			<ul v-if="isNotEmpty(value)">
				<li class="poi" v-for="(e, index) in value" :key="e.id" @click="handlePreview(e.id, index)">
					<div class="ell" :class="{'link-primary': e.isConfirmed === 0, 'color-text-secondary': e.isConfirmed !== 0}">
						{{e.title}}
					</div>
					<span class="published-time" :class="{'link-primary': e.isConfirmed === 0, 'color-text-secondary': e.isConfirmed !== 0}">{{e.publishedTime | formatDate}}</span>
				</li>
			</ul>
			<div class="panel-article-empty" v-else>
				暂无数据！
			</div>
		</div>
		<!-- 通知详情查看 -->
		<el-dialog class="panel-article-dialog" center :visible.sync="dialog.visible" width="90%" v-draggable>
			<span slot="title"><span class="b">{{dialog.title}}</span></span>
			<div class="article-content" v-html="dialog.articleContent">
			</div>
			<div class="pb20">
				<span>附件：</span>
				<el-button type="text" size="mini" @click="handleDownload(e.attachId)" v-for="e in dialog.articleAttachList" :key="e.attachId">{{e.fileName}}</el-button>
			</div>
		</el-dialog>
	</div>
</template>
...
export default {
	...
	methods: {
		...
		async init() {
			try {
				this.value = [];
				this.value = await this.getValue(PANEL.ARTICLE);
			} catch (err) {
				console.error(err);
				this.$message({
					type: 'error',
		...
		handlePreview(articleId, index) {
			fetch
				...
				.then(({ data: { title, articleContent, articleAttachList } }) => {
					...
					this.dialog.articleContent = articleContent;
					this.dialog.articleAttachList = articleAttachList;
