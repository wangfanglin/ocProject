	...
	<div class="panel-container">
		...
		<div class="panel-main">
			...
			<div class="mt5">
				...
				<span class="r b mr5">已支出比例</span>
			</div>
			<!-- 表格 -->
			<el-table class="mt5" size="small" :data="value" height="295px" :cell-class-name="handleCellStyle" :cell-style="TABLE_CELL_STYLE">
				<el-table-column label="单位" prop="agyName" header-align="center" align="left" min-width="130"></el-table-column>
			...
			<el-dialog :visible.sync="dialogVisible" width="400px" v-draggable>
				...
				<el-tabs v-model="activeAce">
					<el-tab-pane label="收入" :name="ACE_TYPE.INCOME">
						<v-tree-input :data="acoList" ref='tree' v-model="incomeAco" placeholder="选择会计科目：" style="width: 100%;">
						</v-tree-input>
					</el-tab-pane>
					<el-tab-pane label="支出" :name="ACE_TYPE.EXPENDITURE">
						<v-tree-input :data="acoList" ref='tree' v-model="expenditureAco" placeholder="选择会计科目：" style="width: 100%;">
						</v-tree-input>
					</el-tab-pane>
				</el-tabs>
...
export default {
	...
	methods: {
		...
		async init() {
			try {
				this.value = [];
				this.value = await this.getValue(PANEL.BUD_BALANCE);
			} catch (err) {
				console.error(err);
				this.$message({
					type: 'error',
